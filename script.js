...
function updateSlider(index) {
  testItems.forEach((item, i) => {
    ...
    if (i === currentIndex) {
      item.style.animation = index > currentIndex ? "next1 0.5s forwards" : "prev1 0.5s forwards";
    }
    if (i === index) {
      item.classList.add("active");
      item.style.animation = index > currentIndex ? "next2 0.5s forwards" : "prev2 0.5s forwards";
    }
  });
